plot3 <- function() {
    ...
    library(tictoc)
    library(datasets)
    message( cat("-------Start Generating Plot 3-------", date()))
    tic("Generate Plot 3")
    ...
    ##run Plot 3 and save as Plot 3 as PNG
    with(power_consumption, plot(Time, Sub_metering_1, type = "l", ylab = "Energy sub meeting", xlab = ""))
    with(power_consumption, lines(Time,Sub_metering_2, col = "red"))
    with(power_consumption, lines(Time,Sub_metering_3, col = "blue"))
    ##legend("topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col=c("black", "red", "blue"), lwd = 3, bty = "o", y.intersp = 0.3)
    legend("topright", bty = "o" , lty = 1, lwd = 3, col=c("black", "red", "blue"), y.intersp = 0.3, legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
    invisible(dev.copy(png,"plot3.png"))
    invisible(dev.off())
    
    toc()
    message( cat("-------Plot 3 is ready-------", date()))
